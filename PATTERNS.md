...
- `COPILOT_GOVERNANCE=/home/h02332/.copilot/governance/COPILOT_GOVERNANCE.md`
- `COPILOT_VIOLATIONS_LOG=/home/h02332/.copilot/violations/session-$(date +%Y%m%d-%H%M%S).jsonl`
...
Shell standards: See ~/.copilot/templates/shell/
Violations log: ~/.copilot/violations/session-YYYYMMDD-HHMMSS.jsonl
```

### Method 3: Reference Governance
```
Reference: ~/.copilot/governance/COPILOT_GOVERNANCE.md
...
3. **Reference pattern**: "See ~/.copilot/enforcement/violation-patterns.md#[ID]"
...
1. **Load profile**: `source ~/.copilot/scripts/load-profile.sh strict-engineering`
...
~/.copilot/scripts/compliance-score.py /tmp/session-metrics.json
```
...
```bash
echo '{"timestamp":"2025-02-07T19:30:00Z","type":"V-MEDIUM-001","severity":"medium","description":"Narrative padding detected","context":"Added 15 unrequested explanation lines","resolution":"Removed padding, applied strict-engineering limits"}' >> ~/.copilot/violations/session-20250207-193000.jsonl
```
...
```bash
~/.copilot/scripts/benchmark-system.sh > baseline-perf.txt

# Make changes

~/.copilot/scripts/benchmark-system.sh > new-perf.txt
...
### ✅ Correct: Minimal Acknowledgment
```
[Immediately start with tool calls to check system state]
```

### ❌ Anti-Pattern: Narrative Padding
```
...
- Shell standards: `~/.copilot/templates/shell/`
- Post-mortems: `~/.copilot/reports/post-mortems/`
