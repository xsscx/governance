...
if [ -f "$HOME/.copilot/profiles/strict-engineering.json" ]; then
    echo "  [OK] PASS: Profile file found"
    ((PASS++))
else
    echo "  [FAIL] FAIL: Profile file not found"
    ((FAIL++))
fi
...
if python3 -c "import json; json.load(open('$HOME/.copilot/profiles/strict-engineering.json'))" 2>/dev/null; then
    echo "  [OK] PASS: Valid JSON"
    ((PASS++))
else
    echo "  [FAIL] FAIL: Invalid JSON"
    ((FAIL++))
fi
...
if $FIELDS_OK; then
    echo "  [OK] PASS: All required fields present"
    ((PASS++))
else
    echo "  [FAIL] FAIL: Missing required fields"
    ((FAIL++))
fi
...
if [ -x "$HOME/.copilot/scripts/load-profile.sh" ]; then
    echo "  [OK] PASS: Script exists and is executable"
    ((PASS++))
else
