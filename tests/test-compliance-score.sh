...
if [ -x "$HOME/.copilot/scripts/compliance-score.py" ]; then
    echo "  [OK] PASS: Script found and executable"
    ((PASS++))
else
    echo "  [FAIL] FAIL: Script missing or not executable"
    ((FAIL++))
fi
...
if echo "$OUTPUT" | grep -q "100/100"; then
    echo "  [OK] PASS: Perfect score calculated correctly"
    ((PASS++))
else
    echo "  [FAIL] FAIL: Expected 100/100, got: $OUTPUT"
    ((FAIL++))
fi
...
if [ "$SCORE" -lt 50 ]; then
    echo "  [OK] PASS: Failed score calculated correctly ($SCORE/100)"
    ((PASS++))
else
    echo "  [FAIL] FAIL: Expected <50, got: $SCORE"
    ((FAIL++))
fi
...
if [ -f "$HOME/.copilot/metrics/session-quality.schema.json" ]; then
    echo "  [OK] PASS: Schema file found"
    ((PASS++))
else
    echo "  [FAIL] FAIL: Schema file not found"
    ((FAIL++))
fi
