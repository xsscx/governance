...
if [ -x "$HOME/.copilot/scripts/check-shell-prologue.sh" ]; then
    echo "  [OK] PASS: Checker script found and executable"
    ((PASS++))
else
    echo "  [FAIL] FAIL: Checker script missing or not executable"
    ((FAIL++))
fi
...
if $HOME/.copilot/scripts/check-shell-prologue.sh 2>/dev/null; then
    echo "  [OK] PASS: Compliant workflow detected correctly"
    ((PASS++))
else
    echo "  [FAIL] FAIL: Compliant workflow flagged as violation"
    ((FAIL++))
fi
...
cd "$TEST_DIR"
if ! $HOME/.copilot/scripts/check-shell-prologue.sh 2>/dev/null; then
    echo "  [OK] PASS: Non-compliant workflow detected"
    ((PASS++))
else
    echo "  [FAIL] FAIL: Non-compliant workflow not detected"
    ((FAIL++))
fi
...
if [ -f "$HOME/.copilot/templates/shell/hoyt-bash-shell-prologue-actions.md" ] && \
   [ -f "$HOME/.copilot/templates/shell/hoyt-powershell-prologue-actions.md" ]; then
    echo "  [OK] PASS: Templates found"
    ((PASS++))
else
    echo "  [FAIL] FAIL: Templates missing"
    ((FAIL++))
fi
