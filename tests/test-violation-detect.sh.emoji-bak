...
if [ "$CRITICAL_COUNT" -ge 3 ]; then
    echo "  ✅ PASS: $CRITICAL_COUNT critical violations documented"
    ((PASS++))
else
    echo "  ❌ FAIL: Only $CRITICAL_COUNT critical violations (expected >=3)"
    ((FAIL++))
fi
...
if [ -f "$HOME/.copilot/reports/fingerprints/INDEX.md" ]; then
    echo "  ✅ PASS: Fingerprints database found"
    ((PASS++))
else
    echo "  ❌ FAIL: Fingerprints database not found"
    ((FAIL++))
fi
...
if [ -f "$HOME/.copilot/enforcement/heuristics.yaml" ]; then
    echo "  ✅ PASS: Heuristics file found"
    ((PASS++))
else
    echo "  ❌ FAIL: Heuristics file not found"
    ((FAIL++))
fi
...
if [ "$REPORT_COUNT" -ge 10 ]; then
    echo "  ✅ PASS: $REPORT_COUNT reports found"
    ((PASS++))
else
    echo "  ❌ FAIL: Only $REPORT_COUNT reports (expected >=10)"
    ((FAIL++))
fi
