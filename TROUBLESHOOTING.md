...
# Check profile
cat ~/.copilot/profiles/strict-engineering.json | python3 -m json.tool
...
ls -la ~/.copilot/profiles/

# Verify JSON syntax
python3 -m json.tool ~/.copilot/profiles/strict-engineering.json
```
...
grep "V-CRITICAL" ~/.copilot/enforcement/violation-patterns.md

# Review session log
cat ~/.copilot/violations/*.jsonl | tail -10
```
...
# Edit profile:
vi ~/.copilot/profiles/strict-engineering.json
...
# Time profile load
time source ~/.copilot/scripts/load-profile.sh strict-engineering
```

**Solutions:**
```bash
# Fix 1: Use fast mode
source ~/.copilot/scripts/enable-performance-mode.sh
...
# See: ~/.copilot/examples/dns-bind-setup.md
```
...
**Fix:** `ls ~/.copilot/profiles/` to see valid names
...
2. Examples: `~/.copilot/examples/`
3. Run tests: `~/.copilot/tests/run-all-tests.sh`
4. Check logs: `~/.copilot/violations/*.jsonl`
